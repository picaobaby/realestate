...
class Themes extends ParentModel
{
    ...
    public static function getDefaultTheme()
    {
        if (!isset(self::$_defaultTheme)) {
            $sql = "SELECT title, additional_view, color_theme, bg_image, json_data FROM {{themes}} WHERE is_default=1";
            $data = Yii::app()->db->createCommand($sql)->queryRow();
            
            if (demo() && !basicVersion()) {
                $tmpData = $tmpTemplate = null;
                
                if (isset($_GET['template']) && array_key_exists($_GET['template'], Themes::getTemplatesList())) {
                    $tmpTemplate = $_GET['template'];
                } elseif (isset(Yii::app()->request->cookies['template']) && isset(Yii::app()->request->cookies['template']->value) && array_key_exists(Yii::app()->request->cookies['template']->value, Themes::getTemplatesList())) {
                    $tmpTemplate = Yii::app()->request->cookies['template']->value;
                }
                                
                if (!empty($tmpTemplate)) {
                    $sql = "SELECT title, additional_view, color_theme, bg_image, json_data FROM {{themes}} WHERE title=:title";
                    $tmpData = Yii::app()->db->createCommand($sql)->queryRow(true, array(':title' => CHtml::encode($tmpTemplate)));
                    
                    if (!empty($tmpData)) {
                        $data = $tmpData;
                        unset($tmpData, $tmpTemplate);
                    }
                }
            }
    ...
    {
        return in_array($this->title, array('atlas', 'basis'));
    }

    public function getCitiesString()
    {
        $citiesId = $this->getFromJson('cities');
        ...
        if ($citiesId) {
            ...
            foreach ($citiesModels as $city) {
                $editUrl = issetModule('location') ? Yii::app()->createUrl('/location/backend/city/update', array('id' => $city->id)) : Yii::app()->createUrl('/apartmentCity/backend/main/update', array('id' => $city->id));

                $delUrl = Yii::app()->createUrl('/themes/backend/main/ajaxPdDelCity', array('id' => $city->id));

                $imgSrc = (isset($city->image) && $city->image) ? $city->image->getSmallThumbLink() : Yii::app()->baseUrl . '/uploads/150x100_no_photo_img.png';

                $str .= '<li key="' . $city->id . '" class="ui-state-default">';
                $str .= '<div class="media">';
                $str .= '<div class="media-left">';
                $str .= '<a href="' . $editUrl . '">';
                $str .= '<img class="media-object" src="' . $imgSrc . '" alt="' . $city->name . '">';
                $str .= '</a>';
                $str .= '</div>';
                $str .= '<div class="media-body">';
                $str .= '<h4 class="media-heading">' . $city->name . '</h4>';
                $str .= CHtml::link('<i class="fa fa-edit"></i> ' . tt('Edit city'), $editUrl, array('target' => '_blank'));
                $str .= '<br/>';
                $str .= CHtml::link('<i class="fa fa-trash"></i> ' . tt('Delete row'), $delUrl, array('class' => 'pd_del', 'onclick' => 'pdDelCity(this.href); return false;'));
                $str .= '</div>';
                $str .= '</div>';
                $str .= '</li>';
            }
