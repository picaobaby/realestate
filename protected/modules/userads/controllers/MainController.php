            ...
            if ($isUpdate) {
                ...
                if ($model->save(false)) {
                    if ($this->_isAPICall) {
                        ...
                        /*
                        # reload model
                        $model = $this->loadModelUserAd($model->id);

                        $imagesList = array();
                        $imagesList['imgCount'] = 0;
                        foreach($model->images as $image) {
                            $imagesList[] = array(
                                'file_name_path' => Images::getFullSizeUrl($image)
                            );
                        }
                        $imagesList['imgCount'] = count($imagesList) - 1;

                        $model->setAttribute('imagesList', $imagesList);
                    ...
                    if ($model->isChild()) {
                        if (isset($model->parent) && !empty($model->parent) && isset($model->parent->objType) && !empty($model->parent->objType)) {
