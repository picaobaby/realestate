...
class MainController extends ModuleAdminController
{
    ...
    public function actionGraph()
    {
        ...
        $sql = 'SELECT COUNT(id) as count, STR_TO_DATE(date_created, "%Y-%m-%d") as date_created FROM {{booking_table}} GROUP BY date_created HAVING date_created >= CURDATE() - INTERVAL ' . self::LAST_DAYS . ' DAY ';
        ...
        $sql = 'SELECT COUNT(id) as count, STR_TO_DATE(date_created, "%Y-%m-%d") as date_created FROM {{apartment}} WHERE active <> ' . Apartment::STATUS_DRAFT . ' GROUP BY date_created HAVING date_created >= CURDATE() - INTERVAL ' . self::LAST_DAYS . ' DAY';
        ...
        if (Yii::app()->user->checkAccess('payment_admin') && issetModule('payment')) {
            $sql = 'SELECT COUNT(id) as count, STR_TO_DATE(date_created, "%Y-%m-%d") as date_created FROM {{payments}} WHERE status = ' . Payments::STATUS_PAYMENTCOMPLETE . ' GROUP BY date_created HAVING date_created >= CURDATE() - INTERVAL ' . self::LAST_DAYS . ' DAY';
        ...
        $sql = 'SELECT COUNT(id) as count, STR_TO_DATE(date_created, "%Y-%m-%d") as date_created FROM {{users}} WHERE active = 1 GROUP BY date_created HAVING date_created >= CURDATE() - INTERVAL ' . self::LAST_DAYS . ' DAY';
        ...
        $sql = 'SELECT COUNT(id) as count, STR_TO_DATE(date_created, "%Y-%m-%d") as date_created FROM {{comments}} GROUP BY date_created HAVING date_created >= CURDATE() - INTERVAL ' . self::LAST_DAYS . ' DAY';
        ...
        $sql = 'SELECT COUNT(id) as count, STR_TO_DATE(date_created, "%Y-%m-%d") as date_created FROM {{reviews}} GROUP BY date_created HAVING date_created >= CURDATE() - INTERVAL ' . self::LAST_DAYS . ' DAY';
