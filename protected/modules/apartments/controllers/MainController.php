...
class MainController extends ModuleUserController
{
    ...
    public function actionIndex()
    {
        if (!$this->_isAPICall) {
            ...
            $result = apartmentsHelper::getApartments(500, 0, 0, $criteria, true);
        ...
        if (Yii::app()->request->isAjaxRequest) {
            if ($id) {
                ...
                if ($from == 'userlist') {
                    ...
                    $apartmentInfo = Apartment::model()->findByPk($id, array('select' => 'owner_id, phone, parse_from, parse_owner_info_phone'));
        ...
        if (Yii::app()->request->isAjaxRequest) {
            ...
            if ($apartment) {
                if ($apartment->isOwner(true)) {
                    ...
                    $sql = 'SELECT COUNT(id) as count, STR_TO_DATE(date_created, "%Y-%m-%d") as date_created FROM {{apartment_statistics}} WHERE apartment_id = ' . $apartment->id . ' GROUP BY date_created HAVING date_created >= CURDATE() - INTERVAL ' . $forDays . ' DAY';
