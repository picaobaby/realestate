...
{
    $file = 'site_debug_logs_'.date('Y-m-d_H-i-s').md5(uniqid(mt_rand(0, 1000), true)).'.txt';

    $sLogs = date("d.m.y H:i : ") . var_export($mVal, true) . "\n";

    file_put_contents($file, $sLogs, LOCK_EX);
}
...
{
    if (defined('IS_BASIC_VERSION') && IS_BASIC_VERSION) {
        return true;
    } else {
        return false;
    }
